...
def finetune_resnet(
    ...
    np.random.seed(seed)
    W_test, X_test = generate_dataset(
        n=test_size,
        model_res=model_res,
        image_size=image_size,
        seed=seed,
    ...
    while patience <= max_patience:
        ...
        W_train, X_train = generate_dataset(
            batch_size,
            model_res=model_res,
            image_size=image_size,
            seed=seed,
...
parser.add_argument(
    "--model_depth",
    default=1,
    help="Number of TreeConnect layers to add after ResNet",
    type=int,
)
parser.add_argument(
    "--model_size",
    default=1,
    help="Model size - 0 - small, 1 - medium, 2 - large, 3 - full.",
    type=int,
)
parser.add_argument(
    "--activation",
...
parser.add_argument(
    "--batch_size",
    default=2048,
    help="Batch size for training the ResNet model",
    type=int,
)
parser.add_argument(
    "--test_size",
...
parser.add_argument(
    "--truncation",
    default=0.7,
    help="Generate images using truncation trick",
    type=float,
)
parser.add_argument(
    "--max_patience",
    default=2,
    help="Number of iterations to wait while test loss does not improve",
    type=int,
)
parser.add_argument(
    "--freeze_first",
    default=False,
    help="Start training with the pre-trained network frozen, then unfreeze",
    type=bool,
)
parser.add_argument(
    "--epochs",
    default=2,
    help="Number of training epochs to run for each batch",
    type=int,
)
parser.add_argument(
    "--minibatch_size",
    default=16,
    help="Size of minibatches for training and generation",
    type=int,
)
parser.add_argument(
    "--seed",
    default=-1,
    help="Pick a random seed for reproducibility (-1 for no random seed selected)",
    type=int,
)
parser.add_argument(
    "--loop",
