...
def tf_custom_adaptive_rgb_loss(a, b):
    ...
    loss, _, _ = image_lossfun(
        b - a, color_space="RGB", representation="PIXEL"
    )
    return tf.math.reduce_mean(loss)
...
def tf_custom_l1_loss(img1, img2):
    return tf.math.reduce_mean(
        tf.math.abs(img2 - img1), axis=None
    )


def tf_custom_logcosh_loss(img1, img2):
    return tf.math.reduce_mean(
        tf.keras.losses.logcosh(img1, img2)
    )
...
class PerceptualModel:
    def __init__(
        ...
        if (
            perc_model is not None
            and self.lpips_loss is not None
        ):
    ...
    def add_placeholder(self, var_name):
        ...
        setattr(
            ...
            tf.compat.v1.placeholder(
                var_val.dtype, shape=var_val.get_shape()
            ),
        )
        setattr(
            self,
            var_name + "_op",
            var_val.assign(
                getattr(self, var_name + "_placeholder")
            ),
        )
    ...
    def build_perceptual_model(
        ...
        incremented_global_step = tf.compat.v1.assign_add(
            global_step, 1
        )
        ...
        with tf.compat.v1.variable_scope(
            "ref_img", reuse=tf.compat.v1.AUTO_REUSE
        ):
        ...
        with tf.compat.v1.variable_scope(
            "ref_weight", reuse=tf.compat.v1.AUTO_REUSE
        ):
        ...
        if self.vgg_loss is not None:
            vgg16 = VGG16(
                include_top=False,
                weights="./babygan/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5",
            ...
            with tf.compat.v1.variable_scope(
                "ref_img_features", reuse=tf.compat.v1.AUTO_REUSE
            ):
            ...
            with tf.compat.v1.variable_scope(
                "features_weight", reuse=tf.compat.v1.AUTO_REUSE
            ):
    ...
    def generate_face_mask(self, im):
        ...
        for (j, rect) in enumerate(rects):
            ...
            if self.use_grabcut:
                ...
                mask = cv2.circle(
                    mask, center, radius, cv2.GC_PR_FGD, -1
                )
    ...
    def set_reference_images(self, images_list):
        assert (
            len(images_list) != 0
            and len(images_list) <= self.batch_size
        )
        ...
        if self.perceptual_model is not None:
            image_features = (
                self.perceptual_model.predict_on_batch(
        ...
        if self.face_mask:
            ...
            for (i, im) in enumerate(loaded_image):
                try:
                    ...
                    print(
                        "Exception in mask handling for "
                        + mask_img
                    )
                    traceback.print_exc()
        ...
        if len(images_list) != self.batch_size:
            if image_features is not None:
                features_space = list(
                    self.features_weight.shape[1:]
                )
                existing_features_shape = [
                    len(images_list)
                ] + features_space
                empty_features_shape = [
                    self.batch_size - len(images_list)
                ] + features_space
                existing_examples = np.ones(
                    shape=existing_features_shape
                )
                empty_examples = np.zeros(
                    shape=empty_features_shape
                )
                weight_mask = np.vstack(
                    [existing_examples, empty_examples]
                )
            ...
            existing_images_space = [
                len(images_list)
            ] + images_space
            empty_images_space = [
                self.batch_size - len(images_list)
            ] + images_space
            existing_images = np.ones(
                shape=existing_images_space
            )
        ...
        if image_features is not None:
            self.assign_placeholder(
                "features_weight", weight_mask
            )
            self.assign_placeholder(
                "ref_img_features", image_features
            )
    ...
    def optimize(
        ...
        if use_optimizer == "lbfgs":
            ...
            min_op = optimizer.minimize(
                self.loss, var_list=[vars_to_optimize]
            )
            self.sess.run(
        ...
        for _ in range(iterations):
            if use_optimizer == "lbfgs":
                optimizer.minimize(
                    self.sess,
                    fetches=[vars_to_optimize, self.loss],
                )
