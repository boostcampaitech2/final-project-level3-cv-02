...
def create_variable_for_generator(
    ...
    if tiled_dlatent:
        ...
        with tf.compat.v1.variable_scope(
            "learnable_dlatents", reuse=tf.compat.v1.AUTO_REUSE
        ):
            learnable_dlatents = tf.compat.v1.get_variable(
                "learnable_dlatents",
...
class Generator:
    def __init__(
        ...
        clipped_values = tf.where(
            clipping_mask,
            tf.random.normal(shape=self.dlatent_variable.shape),
    ...
    def set_dlatents(self, dlatents):
        if self.tiled_dlatent:
            if (dlatents.shape != (self.batch_size, 512)) and (
                dlatents.shape[1] != 512
            ):
                dlatents = np.mean(dlatents, axis=1)
            if dlatents.shape != (self.batch_size, 512):
                dlatents = np.vstack(
                    [dlatents, np.zeros((self.batch_size - dlatents.shape[0], 512))]
                )
            ...
            if isinstance(dlatents.shape[0], int):
                if dlatents.shape != (self.batch_size, self.model_scale, 512):
                    dlatents = np.vstack(
                        [
                            ...
                            np.zeros(
                                (
                                    self.batch_size - dlatents.shape[0],
                                    self.model_scale,
                                    512,
                                )
                            ),
                ...
                self.sess.run(
                    [self._assign_dlantent], {self._assign_dlatent_ph: dlatents}
                )
                return
            else:
