...
def run_align_images(f, m, u2id, P_ROOT):
    ...
    for p in [[P_ROOT + "mother", "mother"], [P_ROOT + "father", "father"]]:
        ...
        if not os.path.isfile(_img):
            raise ValueError(
                "No face was found or there is more than one in the photo."
            )
...
def do_inference(f, m, u2id):
    ...
    if len(os.listdir(osp.join(ROOT, f"{u2id}generated"))) >= 2:
        ...
        raise ValueError(
            "Something wrong. It may be impossible to read the face in the photos. Upload other photos and try again."
        )
    ...
    if style == "Father's photo":
        ...
        hybrid_face = ((1 - genes_influence) * first_face) + (
            genes_influence * second_face
        )
