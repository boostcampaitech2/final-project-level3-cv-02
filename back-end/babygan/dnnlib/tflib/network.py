...
class Network:
    ...
    def __init__(
        ...
        **static_kwargs
        ...
        assert isinstance(
            func_name, str
        ) or util.is_top_level_function(func_name)
        ...
        if util.is_top_level_function(func_name):
            func_name = util.get_top_level_function_name(
                func_name
            )
        (
            module,
            self._build_func_name,
        ) = util.get_module_from_obj_name(func_name)
        ...
        self._build_module_src = _import_module_src.get(
            module, None
        )
    ...
    def _init_fields(self) -> None:
        ...
        self._build_func_name = (
            None  # Name of the build function.
        )
        self._build_module_src = None  # Full source code of the module containing the build function.
        self._run_cache = (
            dict()
        )  # Cached graph data for Network.run().
    ...
    def _init_graph(self) -> None:
        ...
        with tf.name_scope(None):
            self.scope = (
                tf.compat.v1.get_default_graph().unique_name(
        ...
        # Collect outputs.
        assert tfutil.is_tf_expression(out_expr) or isinstance(
            out_expr, tuple
        )
        self.output_templates = (
            [out_expr]
            if tfutil.is_tf_expression(out_expr)
            else list(out_expr)
        )
        ...
        if len(self.components) != len(
            ...
            raise ValueError(
                "Components of a Network must have unique names."
            )

        # List inputs and outputs.
        self.input_shapes = [
            tfutil.shape_to_list(t.shape)
            for t in self.input_templates
        ]
        self.output_shapes = [
            tfutil.shape_to_list(t.shape)
            for t in self.output_templates
    ...
    def reset_own_vars(self) -> None:
        ...
        tfutil.run(
            [var.initializer for var in self.own_vars.values()]
        )

    def reset_vars(self) -> None:
        """Re-initialize all variables of this network, including sub-networks."""
        tfutil.run(
            [var.initializer for var in self.vars.values()]
        )

    def reset_trainables(self) -> None:
        """Re-initialize all trainable variables of this network, including sub-networks."""
        tfutil.run(
            [var.initializer for var in self.trainables.values()]
        )
    ...
    def get_output_for(
        ...
        **dynamic_kwargs
        ...
        with tfutil.absolute_variable_scope(
            ...
            valid_inputs = [
                expr for expr in in_expr if expr is not None
            ]
            final_inputs = []
            for expr, name, shape in zip(
                in_expr, self.input_names, self.input_shapes
            ):
                if expr is not None:
                    expr = tf.identity(expr, name=name)
                else:
                    expr = tf.zeros(
                        [tf.shape(valid_inputs[0])[0]]
                        + shape[1:],
                        name=name,
                    )
                final_inputs.append(expr)
            out_expr = self._build_func(
                *final_inputs, **build_kwargs
            )
    ...
    def get_var_local_name(
        self, var_or_global_name: Union[TfExpression, str]
    ) -> str:
        """Get the local name of a given variable, without any surrounding name scopes."""
        assert tfutil.is_tf_expression(
            var_or_global_name
        ) or isinstance(var_or_global_name, str)
        global_name = (
            var_or_global_name
    ...
    def find_var(
        self, var_or_local_name: Union[TfExpression, str]
    ) -> TfExpression:
        """Find variable by local or global name."""
        assert tfutil.is_tf_expression(
            var_or_local_name
        ) or isinstance(var_or_local_name, str)
        return (
    ...
    def set_var(
        ...
        tfutil.set_vars(
            {self.find_var(var_or_local_name): new_value}
        )
    ...
    def __setstate__(self, state: dict) -> None:
        ...
        exec(
            self._build_module_src, module.__dict__
    ...
    def copy_own_vars_from(self, src_net: "Network") -> None:
        ...
        names = [
            name
            for name in self.own_vars.keys()
            if name in src_net.own_vars
        ]
        tfutil.set_vars(
            tfutil.run(
                {
                    self.vars[name]: src_net.vars[name]
                    for name in names
                }
            )
        )
    ...
    def copy_compatible_trainables_from(
        ...
        for name in self.trainables.keys():
            if name not in src_net.trainables:
                print(
                    "Not restoring (not present):     {}".format(
                        name
                    )
                )
            elif (
                self.trainables[name].shape
                != src_net.trainables[name].shape
            ):
                print(
                    "Not restoring (different shape): {}".format(
                        name
                    )
                )

            if (
                name in src_net.trainables
                and self.trainables[name].shape
                == src_net.trainables[name].shape
            ):
                names.append(name)
    ...
    def apply_swa(self, src_net, epoch):
        ...
        tfutil.set_vars(
            tfutil.run(
                {
                    self.vars[name]: (
                        src_net.vars[name] * scale_new_data
        ...
        with tfutil.absolute_name_scope(
            ...
            for name, var in self.vars.items():
                if name in src_net.vars:
                    cur_beta = (
                        beta
                        if name in self.trainables
                        else beta_nontrainable
                    )
    ...
    def run(
        ...
        **dynamic_kwargs
        ...
        assert (
            input_transform is None
            or util.is_top_level_function(
        ...
        out_arrays = [
            np.empty(
                [num_items]
                + tfutil.shape_to_list(expr.shape)[1:],
                expr.dtype.name,
            )
        ...
        for mb_begin in range(0, num_items, minibatch_size):
            ...
            mb_in = [
                ...
                for src, shape in zip(
                    in_arrays, self.input_shapes
                )
            ]
    ...
    def list_ops(self) -> List[TfExpression]:
        ...
        ops = [
            op
            for op in ops
            if op.name.startswith(include_prefix)
        ]
        ops = [
            op
            for op in ops
            if not op.name.startswith(exclude_prefix)
        ]
        return ops

    def list_layers(
        self,
    ) -> List[Tuple[str, TfExpression, List[TfExpression]]]:
        ...
        def recurse(scope, parent_ops, parent_vars, level):
            ...
            cur_ops = [
                ...
                if op.name.startswith(global_prefix)
            ...
            for var in [
                ...
                cur_ops = [
                    op
                    for op in cur_ops
                    if not op.name.startswith(var_prefix)
                ]
            ...
            if (level == 0 or not contains_direct_ops) and (
                ...
                visited = set()
                for rel_name in [
                    op.name[len(global_prefix) :]
                    for op in cur_ops
                ] + [
                    name[len(local_prefix) :]
                    for name, _var in cur_vars
                ]:
            ...
            layers.append(
                (layer_name, layer_output, layer_trainables)
            )
    ...
    def print_layers(
        self,
        title: str = None,
        hide_layers_with_no_params: bool = False,
    ) -> None:
        ...
        for (
            ...
            weights = [
                ...
                or var.name.endswith("/weight_1:0")
            ]
            ...
            if not hide_layers_with_no_params or num_params != 0:
                num_params_str = (
                    str(num_params) if num_params > 0 else "-"
                )
    ...
    def setup_weight_histograms(self, title: str = None) -> None:
        ...
        with tf.name_scope(None), tf.device(
            None
        ), tf.control_dependencies(None):
...
def _handle_legacy_output_transforms(
    output_transform, dynamic_kwargs
):
    ...
    if not any(
        kwarg in dynamic_kwargs for kwarg in legacy_kwargs
    ):
    ...
    if _print_legacy_warning:
        ...
        print(
            "instead of 'out_mul=127.5, out_add=127.5, out_dtype=np.uint8'."
        )
        print()
