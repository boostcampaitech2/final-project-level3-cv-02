...
class RunContext(object):
    ...
    def __init__(
        ...
        if config_module is not None:
            with open(
                os.path.join(
                    submit_config.run_dir, "config.txt"
                ),
                "w",
            ) as f:
                filtered_dict = {
        ...
        with open(
            os.path.join(submit_config.run_dir, "run.txt"), "w"
        ) as f:
            pprint.pprint(
                self.run_txt_data,
                stream=f,
    ...
    def update(
        ...
        if os.path.exists(
            os.path.join(self.submit_config.run_dir, "abort.txt")
        ):
            self.should_stop_flag = True

        max_epoch_val = (
            self.max_epoch if max_epoch is None else max_epoch
        )
    ...
    def close(self) -> None:
        ...
        if not self.has_closed:
            ...
            with open(
                os.path.join(
                    self.submit_config.run_dir, "run.txt"
                ),
                "w",
            ) as f:
                pprint.pprint(
