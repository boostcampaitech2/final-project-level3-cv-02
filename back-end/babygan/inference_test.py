...
def run_align_images(f,m):
    # run align_images.py with father and mother
    #for p in [f, m]: # 아 알아버렸어
    for p in ["./babygan/hyundong","./babygan/hyundong2"] :
        # init file dir with ROOT path
        _python_file = osp.join(ROOT, "align_images.py") 
        #_src = osp.join(ROOT, f"{p}_image")
        #_src = p # << 'https://12war.s3.ap-northeast-2.amazonaws.com/woman/8373aa58-2985-4e76-a9fe-d2f3deea6a7f차은우.jpeg'
        _src = p
        ...
        # created img file
        _img = osp.join(_dst,'hi_01.png') # 경로.. 나 이제 박사야. 마자 두개다 되고 
        # check if face exist
        if not os.path.isfile(_img): # 여기서 나버리는디 . 
           raise ValueError('No face was found or there is more than one in the photo.')


def run_encode_images():
    # init file dir with ROOT path dddd주석일걸?!
...
def do_inference(f,m): # 메인 지우고 이거로해보림.
    # run align_images.py with mother and father images
    run_align_images(f,m)
    run_encode_images() # 이거시작.
